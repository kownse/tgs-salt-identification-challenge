...
class ResUNet(nn.Module):
    def __init__(self, use_bool=False):
        ...
        self.deboundry_a = Decoder(2560, 512, 64)
        self.deboundry_b = Decoder(1088, 256, 64)
        self.deboundry_c = Decoder(576, 128, 64)
        self.deboundry_d = Decoder(320, 96, 64)
        self.deboundry_e = Decoder(128, 96, 64, upsample=False)

        self.boundry_out = self._mask_out()
    ...
    def forward(self, x):
        ...
        #print('decoders')
        b1 = self.deboundry_a(center, c5)
        #print(d1.size())
        b2 = self.deboundry_b(b1, c4)
        #print(d2.size())
        b3 = self.deboundry_c(b2, c3)
        #print(d3.size())
        b4 = self.deboundry_d(b3, c2)
        #print(d4.size(), c1.size())
        b5 = self.deboundry_e(b4, c1)
        #print(d5.size(), d4.size(), d3.size(), d2.size(), d1.size())
        bhcol = torch.cat([b5, b4,
                          #F.upsample(d4, scale_factor=2, mode='bilinear',
                          #           align_corners=False),
                          F.upsample(b3, scale_factor=2, mode='bilinear', 
                                     align_corners=False),
                          F.upsample(b2, scale_factor=4, mode='bilinear', 
                                     align_corners=False),
                          F.upsample(b1, scale_factor=8, mode='bilinear', 
                                     align_corners=False)], dim=1)

        boundry = self.mask_out(self.dropout(bhcol))
