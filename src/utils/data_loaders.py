...
class MaskDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if not self.valid:
            img_np, msk_np, boundry_np = augment_img([img, msk, boundry], imsize=self.imsize)
        else:
            ...
            boundry_np = reflect_pad(boundry, int((self.imsize - boundry.shape[0]) / 2)) 
...
class MaskTestDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        img = img_as_float(imread('../../input/test/images/' + self.img_ids[index]))[:,:,:3]
        ...
        out_dict = {'img': img_tch,
                    'img_lr': img_lr_tch,
                    'id': self.img_ids[index].replace('.png', ''),
                    'blank': torch.tensor(os.stat('../../input/test/images/' + self.img_ids[index]).st_size != 107)}
...
def get_data_loaders(imsize=128, batch_size=16, num_folds=5, fold=0):
    ...
    img_ids = [os.path.basename(x) for x in glob.glob('../../input/train/images/*.png')]
...
#     small_msk_ids = list(set([os.path.basename(x) for x in glob.glob('../../input/train/small_masks/images/*.png')]) & 
...
#     print('Supersampling {} small masks'.format(len(small_msk_ids)))
    ...
    train_dataset = MaskDataset(imsize=imsize, img_ids=img_ids,
                                  img_paths='../../input/train/images/',
                                  mask_paths='../../input/train/masks/',
                                  small_msk_ids=None)
...
def get_data_mt_loaders(imsize=128, batch_size=16, num_folds=5, fold=0, unlabeled_ratio=0.5):
    ...
    unlabeled_ids = [os.path.basename(x) for x in glob.glob('../../input/test/images/*.png')]
    
    # set up the datasets
    train_dataset = MaskDataset_MT(imsize=imsize, labeled_ids=img_ids,
                                   unlabeled_ids=unlabeled_ids,
                                   unlabeled_ratio=unlabeled_ratio,
                                   labeled_img_paths='../../input/train/images/',
                                   unlabeled_img_paths='../../input/test/images/',
                                   mask_paths='../../input/train/masks/')
