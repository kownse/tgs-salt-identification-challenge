...
from models.nets_boundry import ResUNet
...
def train(net, optimizer, train_loader, freeze_bn=False, use_lovasz=False, swa=False):
    ...
    for i, data in enumerate(train_loader):
        ...
        valid_boundry = data['boundry'].to(device)
...
#         print('imgs.shape', imgs.shape)
        msk_preds, boundry_preds = net(imgs)
#         msk_blend = msk_preds * bool_preds.view(imgs.size(0), 1, 1, 1)
        ...
        if use_lovasz:
            ...
            loss += L.lovasz_hinge(msk_preds, msks)
        loss += args.lambda_boundry * bce(boundry_preds, valid_boundry)
        loss += (1 - args.lambda_boundry) * focal_loss(msk_preds, msks)
...
#             valid_msk_bool = data['has_msk'].float().to(device)
            # get predictions
            msk_vpreds, boundry_vpreds = net(valid_imgs)
#             msk_blend_vpreds = msk_vpreds * bool_vpreds.view(valid_imgs.size(0), 1, 1, 1)
            ...
            vloss += args.lambda_boundry * bce(boundry_vpreds, valid_boundry)
            vloss += (1 - args.lambda_boundry) * focal_loss(msk_vpreds, valid_msks)
